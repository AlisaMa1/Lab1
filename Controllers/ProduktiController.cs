...
namespace OnlineBookStoreApp.Controllers
{
    ...
    public class ProduktiController : Controller
    {
        ...
        {
            ProduktiViewModel productVM = new()
            {
                Produkti = new(),
                ...
                Kopertina = _konteksti.Kopertinat.ToList().Select(i => new SelectListItem
                {
                    Text = i.Emri,
                    Value = i.Id.ToString()
                }),
        ...
        {
            var productList = _konteksti.Produktet.Include(x => x.Kategoria).Include(x => x.Kategoria).ToList();
